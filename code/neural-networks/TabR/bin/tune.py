...
class Config:
    ...
    kfold: KWArgs  # sklearn.model_selection.KFold
    ...
    *,
    ...
    if lib.get_checkpoint_path(output).exists():
        ...
        study = optuna.create_study(
            ...
            storage=f'sqlite:///{str(output/"study.db")}',
        )
        trial_reports = []
        timer = delu.Timer()
    ...
    def objective(trial: optuna.trial.Trial) -> float:
        ...
        kfold = KFold(**C.kfold)
        dataset_info = lib.load_json(lib.get_path(C.space['data']['path']) / 'info.json')
        dummy_X = np.ndarray(
        ...
        for train_idxs, val_idxs in kfold.split(X=dummy_X):
            lib.write_kfold_data(
                lib.get_path(C.space['data']['path']), 
                train_idxs, 
                val_idxs)
            ...
            # If empty, initialise kfold_report with the first report
            # Otherwise, add the metrics to the existing kfold_report metrics
            if not kfold_report:
                kfold_report = report
            else:
                for split, split_metrics in report['metrics'].items():
                    for metric, value in split_metrics.items():
                        kfold_report['metrics'][split][metric] += value

        # Average the metrics over the k folds
        for split, split_metrics in kfold_report['metrics'].items():
            for metric, value in split_metrics.items():
                kfold_report['metrics'][split][metric] /= kfold.get_n_splits(X=dummy_X)

        kfold_report['trial_id'] = trial.number
        kfold_report['tuning_time'] = str(timer)
        trial_reports.append(kfold_report)
        delu.cuda.free_memory()
        return kfold_report['metrics']['val']['score']
