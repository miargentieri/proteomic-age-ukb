...
def write_kfold_data(dataset_dir: Path, 
                     train_idxs: Union[np.ndarray, List[int]], 
                     val_idxs: Union[np.ndarray, List[int]]) -> None:
    
    N = cast(np.ndarray, np.load(dataset_dir / 'X_num_train_val.npy'))
    y = cast(np.ndarray, np.load(dataset_dir / 'Y_train_val.npy'))

    N_train, y_train = N[train_idxs], y[train_idxs]
    np.save(dataset_dir / 'X_num_train.npy', N_train)
    np.save(dataset_dir / 'Y_train.npy', y_train)

    N_val, y_val = N[val_idxs], y[val_idxs]
    np.save(dataset_dir / 'X_num_val.npy', N_val)
