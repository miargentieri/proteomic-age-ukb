...
def objective(trial: optuna.trial.Trial) -> float:
    ...
    kfold = KFold(**args['optimization']['kfold'])
    dataset_info = lib.load_json(lib.get_path(config['data']['path']) / 'info.json')
    dummy_X = np.ndarray(
    ...
    for train_idxs, val_idxs in kfold.split(X=dummy_X):
        ...
        with tempfile.TemporaryDirectory() as dir_:
            ...
            # If this is the first fold, initialize kfold_stats with the
            # first fold's stats. Otherwise, add the current fold's
            # stats['metrics'] to kfold_stats['metrics'].
            if not kfold_stats:
                kfold_stats = stats
            else:
