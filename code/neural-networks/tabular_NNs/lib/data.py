...
def write_kfold_data(dataset_dir: Path, 
                     train_idxs: ty.Union[np.ndarray, ty.List[int]], 
                     val_idxs: ty.Union[np.ndarray, ty.List[int]]) -> None:
    
    N = ty.cast(np.ndarray, np.load(dataset_dir / 'N_train_val.npy'))
    y = ty.cast(np.ndarray, np.load(dataset_dir / 'y_train_val.npy'))

    N_train, y_train = N[train_idxs], y[train_idxs]
    np.save(dataset_dir / 'N_train.npy', N_train)
    np.save(dataset_dir / 'y_train.npy', y_train)

    N_val, y_val = N[val_idxs], y[val_idxs]
    np.save(dataset_dir / 'N_val.npy', N_val)
    np.save(dataset_dir / 'y_val.npy', y_val)

    info = util.load_json(dataset_dir / 'info.json')
    info['train_size'] = len(train_idxs)
    info['val_size'] = len(val_idxs)
    util.dump_json(info, dataset_dir / 'info.json', indent=4)

def write_prediction_kfold_data(dataset_dir: Path, 
                     train_idxs: ty.Union[np.ndarray, ty.List[int]], 
                     val_idxs: ty.Union[np.ndarray, ty.List[int]]) -> None:
    
    N = ty.cast(np.ndarray, np.load(dataset_dir / 'N_all.npy'))
    y = ty.cast(np.ndarray, np.load(dataset_dir / 'y_all.npy'))
    eids = ty.cast(np.ndarray, np.load(dataset_dir / 'eids_all.npy'))

    N_train, y_train, eids_train = N[train_idxs], y[train_idxs], eids[train_idxs]
    np.save(dataset_dir / 'N_train.npy', N_train)
    np.save(dataset_dir / 'y_train.npy', y_train)
    np.save(dataset_dir / 'eids_train.npy', eids_train)

    N_val, y_val, eids_val = N[val_idxs], y[val_idxs], eids[val_idxs]
    np.save(dataset_dir / 'N_val.npy', N_val)
    np.save(dataset_dir / 'y_val.npy', y_val)
    np.save(dataset_dir / 'eids_val.npy', y_val)

    np.save(dataset_dir / 'N_test.npy', N_val)
    np.save(dataset_dir / 'y_test.npy', y_val)
    np.save(dataset_dir / 'eids_test.npy', eids_val)

    info = util.load_json(dataset_dir / 'info.json')
    info['train_size'] = len(train_idxs)
    info['val_size'] = len(val_idxs)
    info['test_size'] = len(val_idxs)
    util.dump_json(info, dataset_dir / 'info.json', indent=4)